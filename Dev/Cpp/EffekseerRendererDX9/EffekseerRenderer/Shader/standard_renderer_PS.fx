...
{
	float4 Output = Input.Color * tex2D(g_sampler[0], Input.UV.xy);
	float4 BlendTexture;
	...
	for (int i = 1; i < 4; i++)
	{
		...
		if (TextureEnable == 1)
		{
			...
			if (i == 1)
			{
				Output.a = tex2D(g_sampler[i], Input.UV.xy).a;
			}

			// 合成画像有
			if (i == 2)
			{
				BlendTexture = tex2D(g_sampler[i], Input.UV.zw);
				BlendEnable = true;
			}
			...
			{
				BlendTexture.a = tex2D(g_sampler[i], Input.UV.zw).a;
			}
		}
	}
